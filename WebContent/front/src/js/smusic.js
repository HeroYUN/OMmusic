    ...
    SmohanMusic.prototype = {
        ...
        createListDom : function(){
            ...
            for(i;i<this.musicLength;i++){
                ul += '<li class="f-toe"><strong>'+this.musicList[i]['title']+'</strong> -- <small>'+this.musicList[i]['singer']+
                '</small><input type="hidden" value="'+i+'"><a id="delete"  style="display:none;float:right;color:black;font-size:24px;margin-right:8px;text-decoration: none;" title="从播放列表移除">x</a></li>';
            }
            ul += '</ul>';
